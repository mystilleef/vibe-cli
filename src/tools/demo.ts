import { resetConstitution, getConstitution } from './constitution.js';
import { vibeGateTool } from './vibeGate.js';
import { vibeLearnTool } from './vibeLearn.js';
import { loadHistory, clearSession } from '../utils/state.js';
...
export async function runDemo({ sessionId = 'demo', modelOverride }: DemoOptions = {}) {
  ...
  // ── Step 1: Constitution ────────────────────────────────────────────────
  const rule = 'Never execute irreversible operations without a tested rollback plan.';

  stepHeader(1, 3, 'Set a constitution rule',
    `vibe constitution set --session ${sessionId} --rule "..."`);

  resetConstitution(sessionId, [rule]);
  ln(dim('  Rule:   ') + rule);
  ln();
  ln(indentJSON({ session: sessionId, rules: getConstitution(sessionId) }));
  ...
  const checkInput = {
    ...
    progress: 'Schema analysis complete. Migration scripts written. Dry-run tested on 1k rows.',
    uncertainties: [
  ...
  stepHeader(2, 3, 'Run a vibe check on a risky plan',
    `vibe check --session ${sessionId} --goal "..." --plan "..." --uncertainty "..."`);
  ...
  const checkResult = await withSpinner('Asking LLM for metacognitive feedback', () =>
  ...
  ln();
  ln(dim('  Decision: ') + (checkResult.proceed ? '✓ proceed' : '✗ blocked') +
    dim('  confidence=') + checkResult.confidence.toFixed(2) +
    dim('  reason=') + checkResult.reason);
  ln(dim('  JSON:') + ' ' + JSON.stringify({ proceed: checkResult.proceed, confidence: checkResult.confidence, reason: checkResult.reason }) );

  // ── Step 3: Learn ───────────────────────────────────────────────────────
  const learnInput = {
    mistake: 'Agent planned a 50M-row schema migration with DROP operations and no rollback strategy.',
    solution: 'Write and test a rollback script before executing any irreversible schema change on production data.',
    category: 'Premature Implementation',
    type: 'mistake' as const,
    sessionId,
  };

  stepHeader(3, 3, 'Record the pattern for future sessions',
  ...
  ln(dim('  Demo data cleared. Use `vibe check` before risky actions, `vibe learn` after mistakes.'));
  ln(dim('  Run `vibe schema` for the full JSON schema for agent integration.'));
  ln();
}
